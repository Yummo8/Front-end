    ...
    <Grid item xs={12} md={4} lg={4}>
      <Card variant="outlined">
        ...
        <CardActions style={{justifyContent: 'flex-end'}}>
          <Button className="shinyButton" component={Link} to={`/${isNode ? 'nodes' : 'vineyard'}/${bank.contract}`}>View</Button>
          <Button disabled={earnings.eq(0)} className={earnings.eq(0) ? 'shinyButtonDisabled' : 'shinyButton'} onClick={onReward}>Claim</Button>
        </CardActions>
      </Card>
    </Grid>
  );
